...
## Kiro CLI でリポジトリを確認する

VS Code のターミナルが、クローンした `aws-cdk-for-workshop` のルートを開いていることを確認してから Kiro CLI を起動します。
...
Kiro CLI で `/context show` を実行し、次を確認します。

- `cdk-contribution-skill` が利用可能である
- リポジトリルートの `AGENTS.md` が context に含まれている

続いて、Kiro CLI に次のメッセージを送ります。

```text
Read AGENTS.md and summarize the workshop target repository, prohibited actions, human approval gates, and validation commands. Do not modify any files yet.
```

回答に次の内容が含まれていることを確認します。

- 対象は `jaws-ug-cdk/aws-cdk-for-workshop`
- `aws/aws-cdk` への Pull Request、Issue、コメントの作成は禁止
- 次の操作の前には参加者の承認が必要
  - フルビルドなどの長時間ビルド
  - integration test の AWS へのデプロイ (実リソースが作成され課金が発生)
  - ブランチの push と Pull Request の作成
- `.contributions/` は Skill の作業ディレクトリで、コミットしない (`.gitignore` 済み)

内容が異なる場合は先へ進まず、ファシリテーターへ確認してください。以降は同じ Kiro CLI セッションで、参加者がコマンドと差分を確認しながら進めます。
...
- integration テストファイル: `packages/@aws-cdk-testing/framework-integ/test/aws-sqs/test/integ.sqs.ts`
