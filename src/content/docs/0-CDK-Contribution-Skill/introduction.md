...
実装計画の作成 (Planning)
    |
    v
参加者による計画の承認  <--- Human approval gate
    |
    v
ビルド・実装 (Build & Implementation)
    |
    +--------+--------+
    |        |        |
    v        v        v
  Test      QA      Docs   (Parallel Validation)
    |        |        |
    +--------+--------+
             |
             v
Security / Regression review (Self Review)
             |
             v
参加者による最終確認    <--- Human approval gate
             |
             v
Pull Request
```
