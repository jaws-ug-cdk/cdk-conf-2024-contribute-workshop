...
### 着手する issue を探したり、プルリクエストを出す内容を決める
...
もしくはもともとコントリビュートしたい内容がある場合はそれでも構いません。しかし、**すでにその内容と同じ issue やプルリクエストが提出されている可能性**もあるので、**いったん既存の issue やプルリクエストを探してみましょう**。

該当する内容の issue やプルリクエストが存在しない場合、issue を起票してからプルリクエストを提出しても良し、そのまま issue を紐付けずにプルリクエストを提出するのもありです。
...
もし、CloudFormation テンプレートに差分があるが、**それが機能追加やバグ修正による想定内の変更である場合**、`--update-on-failed`オプションによって、エラーを抑制し、既存のスナップショットファイルを更新することができます。その際、まず元のスナップショットの状態でのデプロイが実行され、その後、更新後のスナップショットの状態でのデプロイがスタック更新として実行されます。これが成功すればコマンドは正常終了します。
...
特に integration テストに関して、自分の変更した内容に関連するテストファイルがあればそれを変更し、なければ新規ファイルを作成します。

- feat
  - unit テスト
  - integration テスト + snapshots
  - README
- fix
  - unit テスト
  - integration テスト + snapshots
