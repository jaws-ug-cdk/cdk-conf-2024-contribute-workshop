...
この課題では [演習用 Issue #2](https://github.com/jaws-ug-cdk/aws-cdk-for-workshop/issues/2) を利用します。Kiro CLI に次のメッセージを送ってください。

```text
Use the CDK Contribution Skill for https://github.com/jaws-ug-cdk/aws-cdk-for-workshop/issues/2.
Follow AGENTS.md, use only the workshop repository, and stop at every human approval gate.
Analyze the issue and create a plan, but do not reveal or copy the manual solution from the workshop guide.
Do not implement anything until I approve the plan.
```

Issue #1 よりもエージェントへ渡すヒントを減らし、Analysis と Planning の成果物 (`.contributions/2/` 配下) から次を判断します。

- CloudFormation の値を L2 API でどの型として表現するか、その理由が説明されている
- デフォルト値を含む後方互換性が考慮されている
- public API、unit test、integration test、README の変更が計画されている
- 類似実装と AWS CDK の命名・jsii 制約を調査している
- 実際に実行する build、lint、test コマンドが具体的である

設計案は一つとは限りません。エージェントの提案に疑問があれば、代替案とトレードオフを説明させてから計画を承認してください。

実装後は、計画外の変更、生成ファイルの直接編集、過剰なリファクタリングがないか diff を確認します。integration test の AWS デプロイと Pull Request の作成は、それぞれ参加者とファシリテーターの承認後に進めます。
...
yarn integ test/aws-ecr/test/integ.ecr-image-tag-immutability.js --update-on-failed
```
